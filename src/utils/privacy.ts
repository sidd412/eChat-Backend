...
<body>
    <div class="container">
        ...
        <h2>6. Security Measures & Encryption</h2>
        <p>We deploy robust security controls to safeguard your data:</p>
        <ul>
            <li><strong>Data in Transit</strong>: All API communications between the mobile application and our servers are encrypted using Transport Layer Security (TLS/HTTPS) to prevent unauthorized interception.</li>
            <li><strong>Chat Privacy (Encryption at Rest)</strong>: All direct text messages sent within the app are encrypted at rest using the industry-standard AES-256-CBC algorithm. Nobody, including developers or database administrators, can access or read your chats.</li>
            <li><strong>Database & Media Security</strong>: User profiles and metadata are stored on MongoDB databases protected by strict IAM policies. Profile pictures and avatars are securely hosted in Google Cloud Storage with strict access rules.</li>
            <li><strong>RTC Call Security</strong>: Video and audio sessions use dynamic security tokens generated by our Node.js server for every single call to prevent unauthorized eavesdropping.</li>
        </ul>
